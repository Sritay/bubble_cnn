...
def load_model(model, path, device):
    """Loads a model state dictionary from a file if it exists."""
...
def load_mock_trajectories(num_frames=100):
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch+1}: Train Loss: {running_loss/len(train_loader):.4f}, Train Acc: {100*correct/total:.1f}% | Val Loss: {val_loss/len(val_loader):.4f}, Val Acc: {100*val_correct/val_total:.1f}%")
